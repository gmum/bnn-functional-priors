...
try:
    ...
    print(f"[ERROR][TK] Failed to import losses (for closed-form 2-Wasserstein implementation): {e}")
    ...
    def optimize(self, num_iters, n_samples=128, lr=1e-2,
        ...
        for it in range(1, num_iters+1):
            ...
            if (it % print_every == 0) or it == 1:
                # print(f"[TK] X={X.shape} => nnet_samples={nnet_samples.shape} gp_samples={gp_samples.shape}")                            
                self.print_info(">>> Iteration # {:3d}: "
                                "Wasserstein Dist {:.4f}".format(
                                    it, float(wdist)))
                try:
                    batch_target, batch_learning = gp_samples.squeeze(-1).T, nnet_samples.squeeze(-1).T  # [200, 512, 1] --> [512, 200]
                    print(f"[TK] Closed-form 2-Wasserstein = {losses.wasserstein_distance(batch_target, batch_learning)}")
                except Exception as e:
                    print(f"[TK] Failed to calculate closed-form 2-Wasserstein: {e}")
